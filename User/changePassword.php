...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verify old password
    $stm = $_db->prepare('SELECT password FROM member WHERE member_id = ?');
    ...
    if (empty($_err)) {
        $password = sha1($new);
        $update_stm = $_db->prepare('UPDATE member SET password = ? WHERE member_id = ?');
        $result = $update_stm->execute([$password, $member_id]);
...
<div class="change-password-container">
    ...
    <form method="post" class="change-password-form" action="changePassword.php">
    ...
    font-style: normal;
    font-size: 14px;
    text-shadow: none">Password must be:</p>
            <ul>
